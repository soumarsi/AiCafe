...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView
{
    ...
    [UIView animateWithDuration:0.4 animations:^{
        ...
        {
            _chat_table.frame=CGRectMake(_chat_table.frame.origin.x, _chat_table.frame.origin.y, _chat_table.frame.size.width, 320);
            

        }
        else
        {
            _chat_table.frame=CGRectMake(_chat_table.frame.origin.x, _chat_table.frame.origin.y, _chat_table.frame.size.width, 220);
...
//    
    ...
    if (![login_user_id isEqualToString:id_string])
    {
        ...
        time=[[UILabel alloc]initWithFrame:CGRectMake(chat_design.frame.size.width-100,5, 150, 20)];
    ...
    else
    {
        ...
        NSString *date_check=[NSString stringWithFormat:@"%@",[[chat_Data_array objectAtIndex:indexPath.row] objectForKey:@"chat_date"]];
        
        if ([date_check isEqualToString:@"Today"])
        {
        
            time=[[UILabel alloc]initWithFrame:CGRectMake(chat_design.frame.size.width-150,5, 120, 20)];
            time.textColor=[UIColor colorWithRed:(204.0f/255.0f) green:(162.0f/255.0f) blue:(102.0f/255.0f) alpha:1];
            time.font=[UIFont fontWithName:@"OpenSans-Semibold" size:15];
            time.textAlignment=NSTextAlignmentLeft;
            NSString *str=[NSString stringWithFormat:@"%@",[[chat_Data_array objectAtIndex:indexPath.row] objectForKey:@"chat_time"]];
            str=[str substringWithRange:NSMakeRange(0,5)];
            NSString *am=@"AM";
            NSString *pm=@"PM";
            NSInteger timecheck=[str integerValue];
            //NSLog(@"###########%ld",(long)timecheck);
            if(timecheck>12)
            {
            time.text=[NSString stringWithFormat:@"%@ %@ %@",[[chat_Data_array objectAtIndex:indexPath.row] objectForKey:@"chat_date"],str,pm];
            [chat_design addSubview:time];
            }
            else
            {
                time.text=[NSString stringWithFormat:@"%@ %@ %@",[[chat_Data_array objectAtIndex:indexPath.row] objectForKey:@"chat_date"],str,am];
                [chat_design addSubview:time];
            }
        ...
        {
            time=[[UILabel alloc]initWithFrame:CGRectMake(chat_design.frame.size.width-140,5, 120, 20)];
            time.textColor=[UIColor colorWithRed:(204.0f/255.0f) green:(162.0f/255.0f) blue:(102.0f/255.0f) alpha:1];
            time.font=[UIFont fontWithName:@"OpenSans-Semibold" size:15];
            time.textAlignment=NSTextAlignmentLeft;
            time.text=[NSString stringWithFormat:@"%@",[[chat_Data_array objectAtIndex:indexPath.row] objectForKey:@"chat_date"]];
...
-(void)Load_url
{
    ...
    RS_JsonClass *globalobj=[[RS_JsonClass alloc]init];
        ...
        NSLog(@"#### ..... TEST BOT...%@",[NSString stringWithFormat:@"%@sendChatUser.php?send_id=%@&rec_id=%@&message=%@",App_Domain_Url,login_user_id,_getuser_id,chattext]);
        
        NSString *urlstring=[NSString stringWithFormat:@"%@sendChatUser.php?send_id=%@&rec_id=%@&message=%@",App_Domain_Url,login_user_id,_getuser_id,chattext];
